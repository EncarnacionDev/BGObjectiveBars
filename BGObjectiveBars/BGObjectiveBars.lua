...
-----------------------------------------------------------------
local function UpdateBarInfo(statusBar, currentValue, maxValue, formattedText, canReachZero)
	local _, max = statusBar:GetMinMaxValues()
	currentValue = tonumber(currentValue)
	maxValue = tonumber(maxValue)

	if not canReachZero and currentValue == 0 and statusBar.value and statusBar.value > 0 then
		return -- 0 transitorio (muerte) en BGs de puntos: conservar el último valor real
	end
			...
			if statusBar then
				...
				elseif mapID == BATTLEGROUND_ALTERAC_VALLEY then
					...
					if resourceCurrent then
						UpdateBarInfo(statusBar, resourceCurrent, 500, resourceCurrent, true)
						isShownWidgetWorldState = true
					end
				elseif mapID == BATTLEGROUND_ISLE_OF_CONQUEST then
					local resourceCurrent = string.match(text, "(%d+)$")
					if resourceCurrent then
						UpdateBarInfo(statusBar, resourceCurrent, 300, resourceCurrent, true)
						isShownWidgetWorldState = true
					end
...
-- Keep the bars visible while the world map is open (the map hides UIParent frames)
local function HookWorldMap()
	if WorldMapFrame and not WorldMapFrame._bgObjectiveBarsHooked then
		WorldMapFrame._bgObjectiveBarsHooked = true
		WorldMapFrame:HookScript("OnShow", function()
			if bars.frame then bars.frame:Show() end
		end)
		WorldMapFrame:HookScript("OnHide", function()
			if bars.frame then UpdateObjective() end
		end)
	end
end
HookWorldMap()
